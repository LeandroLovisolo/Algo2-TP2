...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iCrearÁrbol}{\In{raiz}{categoria}}{estr\_acat}{$\Theta(|raiz|)$}{
    $\Theta(1) + \Theta(1) + \Theta(1) + \Theta(1) + \Theta(|raiz| + copy(raiz)) + \Theta(1) =$ \\
    $5 * \Theta(1) + \Theta(|raiz| + copy(raiz)) =$ \\
    $\Theta(|raiz| + copy(raiz)) =$ \\
    $\Theta(|raiz|) + \Theta(copy(raiz)) =$ \\
    ...
    \STATE \NombreFuncion{Definir}($raiz$, $estr\_raiz$, $categorias$) \CompTheta{|$raiz$| $+$ $copy(raiz)$}
  ...
  \begin{Algoritmo}{iAgregarCategoría}{\In{hija}{categoria}, \In{padre}{categoria}, \Inout{ac}{estr\_acat}}{}{$\Theta(???)$}{PENDIENTE.}
    \STATE var $estr\_padre$: \TipoVariable{estr\_cat} $\leftarrow$ \NombreFuncion{Obtener}($padre$, $ac$.categorías) \CompTheta{???}
    \STATE \CrearTupla{var $estr\_hija$: \TipoVariable{estr\_cat}}\CampoTupla{id}{\NombreFuncion{\#Claves}($ac$.categorías) + 1}, \CompTheta{???}
    \STATE \MargenTupla                                           \CampoTupla{nombre}{$hija$}, \CompTheta{???}
  \STATE \MargenTupla                                           \CampoTupla{hijos}{\NombreFuncion{Vacío}()}\FinTupla \CompTheta{???}
  \STATE \NombreFuncion{Definir}($hija$, $estr\_hija$, $ac$) \CompTheta{???}     
    \STATE \NombreFuncion{AgregarRápido}($estr\_padre$.hijos, \&(\NombreFuncion{Obtener}($hija$, $ac$.categorías))) \CompTheta{???}
  \end{Algoritmo}

  ~   
    
  \begin{Algoritmo}{iIdCategoríaPorNombre}{\In{c}{categoria}, \In{ac}{estr\_acat}}{nat}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{Obtener}($c$, $ac$.categorías).id \CompTheta{???}
  \end{Algoritmo}

  ~
  
  \begin{Algoritmo}{i\#Categorías}{\In{ac}{estr\_acat}}{nat}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{\#Claves}($ac$.categorías) \CompTheta{???}
  \end{Algoritmo}

  ~

  \begin{Algoritmo}{iCrearIt}{\In{padre}{categoria}, \In{ac}{acat}}{itConj(puntero(estr\_cat))}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{CrearIt}(\NombreFuncion{Obtener}($padre$, $ac$.categorías).hijos) \CompTheta{???}
  \end{Algoritmo}

  ~

  \begin{Algoritmo}{iCrearItRaíz}{\In{ac}{acat}}{itConj(puntero(estr\_cat))}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{CrearIt}($ac$.raíz.hijos) \CompTheta{???}
  \end{Algoritmo}

  ~
  
  \begin{Algoritmo}{iCrearItDadoOtroIt}{\In{it}{itConj(puntero(estr\_cat))}}{itConj(puntero(estr\_cat))}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{CrearIt}(\NombreFuncion{Siguiente}(it)\DRef hijos) \CompTheta{???}
  \end{Algoritmo}  

  ~  
  
  \begin{Algoritmo}{iHayMás?}{\In{it}{itConj(puntero(estr\_cat))}}{bool}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{HaySiguiente}($it$) \CompTheta{???}
  \end{Algoritmo}
  
  ~
  
  \begin{Algoritmo}{iCategoríaActual}{\In{it}{itConj(puntero(estr\_cat))}}{categoria}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{Siguiente}($it$)\DRef nombre \CompTheta{???}
  \end{Algoritmo}

  ~
  
  \begin{Algoritmo}{iIdCategoríaActual}{\In{it}{itConj(puntero(estr\_cat))}}{nat}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{Siguiente}($it$)\DRef id \CompTheta{???}
  \end{Algoritmo}  
  
  ~
  
  \begin{Algoritmo}{iAvanzar}{\Inout{it}{itConj(puntero(estr\_cat))}}{}{$\Theta(???)$}{PENDIENTE.}
    \STATE \NombreFuncion{Avanzar}($it$) \CompTheta{???}
  \end{Algoritmo}
...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iCrearSistema}{\In{ac}{estr\_acat}}{estr\_sistema}{$\Theta(???)$}{PENDIENTE.}
    \STATE \CrearTupla{$res$}\CampoTupla{categorías}{$ac$}, \CompTheta{???}
    \STATE \MargenTupla      \CampoTupla{links}{\NombreFuncion{CrearDiccionario}()}, \CompTheta{???}
    \STATE \MargenTupla      \CampoTupla{linksPorCatId}{\NombreFuncion{CrearArreglo}(\NombreFuncion{\#Categorías}($ac$))}, \CompTheta{???}
    \STATE \MargenTupla      \CampoTupla{fechaActual}{0}\FinTupla \CompTheta{???}
    \STATE \CrearTupla{$res$.linksPorCatId[0]}\CampoTupla{cat}{\NombreFuncion{NombreCategoríaRaíz}($ac$)}, \CompTheta{???}
    \STATE \MargenTupla                       \CampoTupla{idPadre}{0}, \CompTheta{???}
    \STATE \MargenTupla                       \CampoTupla{links}{\NombreFuncion{Vacía}()}, \CompTheta{???}
    \STATE \MargenTupla                       \CampoTupla{últimoAcceso}{0}, \CompTheta{???}
    \STATE \MargenTupla                       \CampoTupla{ordenado?}{\textbf{false}}\FinTupla \CompTheta{???}
    \STATE \NombreFuncion{iAgregarALinksPorCatId(CrearItRaíz($ac$), 1, $res$)}  \CompTheta{???}
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iAgregarALinksPorCatId}{\Inout{it}{itcats}, \In{idPadre}{nat}, \Inout{s}{estr\_sistema}}{}{$\Theta(???)$}{PENDIENTE.}
    \WHILE{\NombreFuncion{HayMás?}($it$)}
      \STATE \CrearTupla{$s$.linksPorCatId[\NombreFuncion{IdCategoríaActual}($it$) - 1]}%
                          \CampoTupla{cat}{\NombreFuncion{CategoríaActual}($it$)},  \CompTheta{???}
      \STATE \MargenTupla \CampoTupla{idPadre}{$idPadre$},  \CompTheta{???}
      \STATE \MargenTupla \CampoTupla{links}{\NombreFuncion{Vacía}()},  \CompTheta{???}
      \STATE \MargenTupla \CampoTupla{últimoAcceso}{0},  \CompTheta{???}
      \STATE \MargenTupla \CampoTupla{ordenado?}{\textbf{false}}\FinTupla  \CompTheta{???}
      \STATE \NombreFuncion{iAgregarALinksPorCatId}(\NombreFuncion{CrearItDadoOtroIt}($it$), \NombreFuncion{IdCategoríaActual}($it$), $s$) \CompTheta{???}
      \STATE \NombreFuncion{Avanzar}($it$) \CompTheta{???}
    \ENDWHILE
  \end{Algoritmo}
  ...
  \begin{Algoritmo}{iAgregarLink}{\In{l}{link}, \In{c}{categoria}, \Inout{s}{estr\_sistema}}{}{$\Theta(???)$}{PENDIENTE.}
    \STATE \CrearTupla{var $estr\_l$: \TipoVariable{estr\_link}}
                        \CampoTupla{l}{$l$}, \CompTheta{???}
    \STATE \MargenTupla \CampoTupla{cid}{\NombreFuncion{IdCategoríaPorNombre}($c$, $s$.categorías)}, \CompTheta{???}
    \STATE \MargenTupla \CampoTupla{últimoAcceso}{$s$.fechaActual},  \CompTheta{???}
    \STATE \MargenTupla \CampoTupla{as}{\NombreFuncion{iCrearArregloDe3Nats}(0, 0, 0)}\FinTupla \CompTheta{???}
    \STATE \NombreFuncion{Definir}($l$, $estr\_l$, $s$.links) \CompTheta{???}
    \STATE $estr\_l$ $\leftarrow$ \NombreFuncion{Obtener}($l$, $s$.links) \CompTheta{???}
    \STATE
    \STATE var $cid$: \TipoVariable{nat} $\leftarrow$ $estr\_l$.cid \CompTheta{???}
    \WHILE{$¬$($cid$ $\igobs$ 0)}
      \STATE \NombreFuncion{AgregarAtrás}($s$.linksPorCatId[$cid$ - 1].links, \&($estr\_l$)) \CompTheta{???}
      \STATE $s$.linksPorCatId[$cid$ - 1].últimoAcceso $\leftarrow$ $s$.fechaActual \CompTheta{???}
      \STATE $s$.linksPorCatId[$cid$ - 1].ordenado? $\leftarrow$ \textbf{false} \CompTheta{???}
      \STATE $cid$ $\leftarrow$ $s$.linksPorCatId[$cid$ - 1].idPadre \CompTheta{???}
    \ENDWHILE
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iAccederLink}{\In{l}{link}, \In{f}{fecha}, \Inout{s}{estr\_sistema}}{}{$\Theta(???)$}{PENDIENTE.}
    \STATE var $estr\_l$: \TipoVariable{estr\_link} $\leftarrow$ \NombreFuncion{Obtener}($l$, $s$.links) \CompTheta{???}
    \IF{$f$ $\igobs$ $estr\_l$.últimoAcceso}
      \STATE $estr\_l$.as[0] $\leftarrow$ $estr\_l$.as[0] + 1 \CompTheta{???}
    \ELSIF{$f$ $\igobs$ $estr\_l$.últimoAcceso + 1}
      \STATE $estr\_l$.as $\leftarrow$ \NombreFuncion{iCrearArregloDe3Nats}(1, $estr\_l$.as[0], $estr\_l$.as[1]) \CompTheta{???}
    \ELSIF{$f$ $\igobs$ $estr\_l$.últimoAcceso + 2}
      \STATE $estr\_l$.as $\leftarrow$ \NombreFuncion{iCrearArregloDe3Nats}(1, 0, $estr\_l$.as[0]) \CompTheta{???}
    \ELSE
      \STATE $estr\_l$.as $\leftarrow$ \NombreFuncion{iCrearArregloDe3Nats}(1, 0, 0) \CompTheta{???}
    \ENDIF
    \STATE
    \STATE $s$.fechaActual $\leftarrow$ $f$ \CompTheta{???}
    \STATE
    \STATE var $cid$: \TipoVariable{nat} $\leftarrow$ $estr\_l$.cid \CompTheta{???}
    \WHILE{$¬$($cid$ $\igobs$ 0)}
      \STATE $s$.linksPorCatId[$cid$ - 1].últimoAcceso $\leftarrow$ $s$.fechaActual \CompTheta{???}
      \STATE $s$.linksPorCatId[$cid$ - 1].ordenado? $\leftarrow$ \textbf{false} \CompTheta{???}
      \STATE $idPadre$ $\leftarrow$ $s$.linksPorCatId[$cid$ - 1].idPadre \CompTheta{???}
    \ENDWHILE     
  \end{Algoritmo}
  
  ~
  
  \begin{Algoritmo}{iCrearArregloDe3Nats}{\In{a}{nat}, \In{b}{nat}, \In{c}{nat}}{arreglo\_estático[3] de nat}{$\Theta(???)$}{PENDIENTE.}
    \STATE $\res$ $\leftarrow$ \NombreFuncion{CrearArreglo}() \CompTheta{???}
    \STATE $\res$[0] $\leftarrow$ $a$ \CompTheta{???}
    \STATE $\res$[1] $\leftarrow$ $b$ \CompTheta{???}
    \STATE $\res$[2] $\leftarrow$ $c$ \CompTheta{???}
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{i\#Links}{\In{c}{categoría}, \In{s}{estr\_sistema}}{nat}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{Longitud}($s$.linksPorCatId[\NombreFuncion{IdCategoríaPorNombre}($c$, $s$.categorías) - 1].links) \CompTheta{???}
  \end{Algoritmo}

  
  ~  
  
  \begin{Algoritmo}{iCrearIt}{\In{c}{categoría}, \In{s}{estr\_sistema}}{estr\_iter}{$\Theta(???)$}{PENDIENTE.}
    \STATE var $estr\_c$: \TipoVariable{estr\_linksPorCatId} $\leftarrow$ $s$.linksPorCatId[\NombreFuncion{IdCategoríaPorNombre}($c$, $s$.categorías) - 1] \CompTheta{???}
    \IF{$¬$($estr\_c$.ordenado?)}
      \STATE \NombreFuncion{iOrdenarLinks}($estr\_c$) \CompTheta{???}
    \ENDIF
    \STATE \CrearTupla{$res$}\CampoTupla{s}{$s$}, \CompTheta{???}
    \STATE \MargenTupla      \CampoTupla{cid}{\NombreFuncion{IdCategoríaPorNombre}($c$, $s$.categorías)}, \CompTheta{???}
    \STATE \MargenTupla      \CampoTupla{it}{\NombreFuncion{CrearIt}($estr\_c$.links)}\FinTupla \CompTheta{???}
  \end{Algoritmo}  
  
  ~
  
  \begin{Algoritmo}{iOrdenarLinks}{\In{estr\_c}{estr\_linksPorCatId}}{}{$\Theta(???)$}{PENDIENTE.}
    \STATE var $it$: \TipoVariable{itLista(puntero(estr\_link))} $\leftarrow$ \NombreFuncion{CrearIt}($estr\_c$.links) \CompTheta{???}
    \WHILE{\NombreFuncion{HaySiguiente}($it$)}
      \STATE var $it'$: \TipoVariable{itLista(puntero(estr\_link))} $\leftarrow$ \NombreFuncion{Copiar}($it$) \CompTheta{???}
      \STATE var $itMax$: \TipoVariable{itLista(puntero(estr\_link))} $\leftarrow$ \NombreFuncion{Copiar}($it'$) \CompTheta{???}
      \WHILE{\NombreFuncion{HaySiguiente}($it'$)}
        \IF{\NombreFuncion{iAccesosRecientes}(*\NombreFuncion{Siguiente}($it'$), $estr\_c$) $>$
          ...
          \STATE $itMax$ $\leftarrow$ \NombreFuncion{Copiar}($it'$) \CompTheta{???}
        \ENDIF
    \STATE \NombreFuncion{Avanzar($it'$)} \CompTheta{???}
    \ENDWHILE
    \STATE \NombreFuncion{iIntercamabiar}($it$, $itMax$) \CompTheta{???}
      \STATE \NombreFuncion{Avanzar($it$)} \CompTheta{???}
    \ENDWHILE
    \STATE $estr\_c$.ordenado? $\leftarrow$ \textbf{true} \CompTheta{???}
  \end{Algoritmo}  
  
  ~  

  \begin{Algoritmo}{iAccesosRecientes}{\In{estr\_l}{estr\_link}, \In{estr\_c}{estr\_linksPorCatId}}{nat}{$\Theta(???)$}{PENDIENTE.}
    \IF{$estr\_c$.últimoAcceso $\igobs$ $estr\_l$.últimoAcceso}
      \STATE $res$ $\leftarrow$ $estr\_l$.as[0] + $estr\_l$.as[1] + $estr\_l$.as[2] \CompTheta{???}
    \ELSIF{$estr\_c$.últimoAcceso $\igobs$ $estr\_l$.últimoAcceso + 1}
      \STATE $res$ $\leftarrow$ $estr\_l$.as[0] + $estr\_l$.as[1] \CompTheta{???}
    \ELSIF{$estr\_c$.últimoAcceso $\igobs$ $estr\_l$.últimoAcceso + 2}          
      \STATE $res$ $\leftarrow$ $estr\_l$.as[0] \CompTheta{???}
    \ELSE
      \STATE $res$ $\leftarrow$ 0 \CompTheta{???}
    \ENDIF
  \end{Algoritmo}
  
  ~
  
  \begin{Algoritmo}{iIntercambiar}{\Inout{it}{itLista(puntero(estr\_link))}, \Inout{it'}{itLista(puntero(estr\_link))}}{}{$\Theta(???)$}{PENDIENTE.}
    \STATE var $ptr$: \TipoVariable{puntero(estr\_link)} $\leftarrow$ \NombreFuncion{Siguiente}($it'$) \CompTheta{???}
    \STATE \NombreFuncion{EliminarSiguiente}($it'$) \CompTheta{???}
    \STATE \NombreFuncion{AgregarComoSiguiente}($it'$, \NombreFuncion{Siguiente}($it$)) \CompTheta{???}
    \STATE \NombreFuncion{EliminarSiguiente}($it$) \CompTheta{???}
    \STATE \NombreFuncion{AgregarComoSiguiente}($it$, $ptr$) \CompTheta{???}
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iHayMás?}{\In{it}{estr\_iter}}{bool}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{HaySiguiente}($it$.it) \CompTheta{???}
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iLinkActual}{\In{it}{estr\_iter}}{link}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{Siguiente}($it$.it)\DRef l \CompTheta{???}
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iCategoríaLinkActual}{\In{it}{estr\_iter}}{categoria}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ $it$.s.linksPorCatId[\NombreFuncion{Siguiente}($it$.it)\DRef cid - 1].cat \CompTheta{???}
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iAccesosRecientesLinkActual}{\In{it}{estr\_iter}}{nat}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ \NombreFuncion{iAccesosRecientes}(*\NombreFuncion{Siguiente}($it$.iter), $it$.s.linksPorCatId[$it$.cid - 1]) \CompTheta{???}
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iAvanzar}{\Inout{it}{estr\_iter}}{}{$\Theta(???)$}{PENDIENTE.}
    \STATE \NombreFuncion{Avanzar}($it$.it) \CompTheta{???}
  \end{Algoritmo}
...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iCrearDiccionario}{}{estr\_dicctrie}{$\Theta(???)$}{PENDIENTE.}
    \STATE \CrearTupla{$res$}\CampoTupla{raíz}{\NombreFuncion{iNuevoNodo}()}, \CompTheta{???}
    \STATE \MargenTupla      \CampoTupla{\#Claves}{$0$}\FinTupla \CompTheta{???}
  \end{Algoritmo}

  ~

  \begin{Algoritmo}{iNuevoNodo}{}{estr\_nodo}{$\Theta(???)$}{PENDIENTE.}
    \STATE \CrearTupla{$res$}\CampoTupla{significado}{NULL}, \CompTheta{???}
    \STATE \MargenTupla      \CampoTupla{hijos}{\NombreFuncion{CrearArreglo}()}\FinTupla \CompTheta{???}
    \FOR{var $i$: \TipoVariable{nat} $\leftarrow$ $0$ to $255$}
      \STATE $res$.hijos[$i$] $\leftarrow$ NULL \CompTheta{???}
    \ENDFOR 
  \end{Algoritmo}

  ~
   
  \begin{Algoritmo}{iDefinir}{\In{c}{string}, \In{s}{$\alpha$}, \Inout{d}{estr\_dicctrie}}{}{$\Theta(???)$}{PENDIENTE.}
    \STATE var $actual$: \TipoVariable{puntero(estr\_nodo)} $\leftarrow$ \&($d$.raíz) \CompTheta{???}
    \FOR{var $i$: \TipoVariable{nat} $\leftarrow$ $0$ to \NombreFuncion{Longitud}($c$)}
      \IF{$actual$\DRef hijos[\NombreFuncion{Ord}($c$[$i$])] $\igobs$ NULL}
        \STATE $actual$\DRef hijos[\NombreFuncion{Ord}($c$[$i$])] $\leftarrow$ \&(\NombreFuncion{iNuevoNodo}()) \CompTheta{???}
      \ENDIF
        \STATE $actual$ $\leftarrow$ $actual$\DRef hijos[\NombreFuncion{Ord}($c$[$i$])] \CompTheta{???}
    \ENDFOR
    \STATE $actual$\DRef significado $\leftarrow$ \&(Copiar($s$)) \CompTheta{???}
    \STATE $d$.\#Claves $\leftarrow$ $d$.\#Claves $+$ $1$ \CompTheta{???}
  \end{Algoritmo}
  
  ~

  \begin{Algoritmo}{iObtener}{\In{c}{string}, \In{d}{estr\_dicctrie}}{$\alpha$}{$\Theta(???)$}{PENDIENTE.}
    \STATE var $actual$: \TipoVariable{puntero(estr\_nodo)} $\leftarrow$ \&($d$.raíz) \CompTheta{???} 
    \FOR{var $i$: \TipoVariable{nat} $\leftarrow$ $0$ to \NombreFuncion{Longitud}($c$)}
      \STATE $actual$ $\leftarrow$ $actual$\DRef hijos[\NombreFuncion{Ord}($c$[$i$])] \CompTheta{???}
    \ENDFOR   
    \STATE $res$ $\leftarrow$ *($actual$\DRef $significado$) \CompTheta{???}
  \end{Algoritmo}

  ~
  
  \begin{Algoritmo}{i\#Claves}{\In{d}{estr\_dicctrie}}{nat}{$\Theta(???)$}{PENDIENTE.}
    \STATE $res$ $\leftarrow$ $d$.\#Claves \CompTheta{???}
  \end{Algoritmo}
