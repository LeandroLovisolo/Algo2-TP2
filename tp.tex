...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iCrearSistema}{\In{ac}{estr\_acat}}{estr\_sistema}
    ...
    \STATE \begin{CrearTupla}{$res$.linksPorCatId[0]}
           ...
           \end{CrearTupla}
    \STATE \NombreFuncion{iAgregarALinksPorCatId(CrearIterHijosDeLaRaíz($ac$), 1, $res$)}   
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iAgregarALinksPorCatId}{\Inout{it}{itercathijos}, \In{idPadre}{nat}, \Inout{s}{estr\_sistema}}{}
    \WHILE{\NombreFuncion{HayMásHijos?}($it$)}
      \STATE \begin{CrearTupla}{$s$.linksPorCatId[\NombreFuncion{IdHijoActual}($it$) - 1]}
               \CampoTupla{cat}{\NombreFuncion{HijoActual}($it$)},
               \CampoTupla{idPadre}{$idPadre$},
               \CampoTupla{links}{\NombreFuncion{Vacía}()},\\
               \CampoTupla{últimoAcceso}{0},
               \CampoTupla{ordenado?}{\textbf{false}}%
             \end{CrearTupla}
      \STATE \NombreFuncion{iAgregarALinksPorCatId}(\NombreFuncion{CrearIterHijosDadoOtroIter}($it$), \NombreFuncion{IdHijoActual}($it$), $s$)
