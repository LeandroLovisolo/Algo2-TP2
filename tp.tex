...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iCrearIterCat}{\In{padre}{categoria}, \In{ac}{estr\_acat}}{itConj(puntero(datoscat))}
  	\STATE $res$ $\leftarrow$ \NombreFuncion{CrearIt}(\NombreFuncion{Obtener}($padre$, $ac$.categorías)\DRef hijos)
  \end{Algoritmo}

  ~  
    
  \begin{Algoritmo}{iIdCategoríaPorNombre}{\In{c}{categoria}, \In{ac}{estr\_acat}}{nat}
  	\STATE $res$ $\leftarrow$ \NombreFuncion{Obtener}($c$, $ac$.categorías)\DRef id
  \end{Algoritmo}

  ~

  \begin{Algoritmo}{iObtenerId}{\In{dc}{estr\_datoscat}}{nat}
  	\STATE $res$ $\leftarrow$ $dc$.id
  \end{Algoritmo}
  
  ~
  
  \begin{Algoritmo}{iObtenerNombre}{\In{dc}{estr\_datoscat}}{nat}
  	\STATE $res$ $\leftarrow$ $dc$.nombre
  \end{Algoritmo}

  ~
    
  \begin{Algoritmo}{iObtenerPadre}{\In{dc}{estr\_datoscat}}{puntero(datoscat)}
  	\STATE $res$ $\leftarrow$ $dc$.padre
  \end{Algoritmo}

  ~

  \begin{Algoritmo}{iObtenerHijos}{\In{dc}{estr\_datoscat}}{conj(puntero(datoscat))}
  	\STATE $res$ $\leftarrow$ $dc$.hijos
  \end{Algoritmo}

  ~

  \begin{Algoritmo}{iHayMás?}{\In{it}{itConj(puntero(datoscat))}}{bool}
  	\STATE $res$ $\leftarrow$ \NombreFuncion{HaySiguiente}($it$)
  \end{Algoritmo}

  ~
  
  \begin{Algoritmo}{iActual}{\In{it}{itConj(puntero(datoscat))}}{puntero(datoscat)}
  	\STATE $res$ $\leftarrow$ \NombreFuncion{Actual}($it$)
  \end{Algoritmo}
  
  ~
  
  \begin{Algoritmo}{iAvanzar}{\Inout{it}{itConj(puntero(datoscat))}}{}
...
\begin{Representacion}
  ...
  \begin{Estructura}{dicctrie$(\alpha)$}[estr\_dicctrie]
    \begin{Tupla}[estr\_dicctrie	]
      \tupItem{nodos}{arreglo\_estático[256] de puntero(estr\_dicctrie)}
...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iCrearDiccionario}{}{estr\_dicctrie}
	...
	\STATE endFor
    \STATE $res$.significado $\leftarrow$ Null
  \end{Algoritmo}

  ~  
   
  \begin{Algoritmo}{iObtener}{\In{c}{string}, \In{e}{estr\_dicctrie}}{$\alpha$}
  	\STATE var $nodoActual$: \TipoVariable{puntero(estr\_dicctrie)} $\leftarrow$  \NombreFuncion{\&}($e$)
  ...
  \begin{Algoritmo}{iDefinir}{\In{c}{string}, \In{s}{$\alpha$}, \In{e}{estr\_dicctrie}}{}
  	\STATE var $nodoActual$: \TipoVariable{puntero(estr\_dicctrie)} $\leftarrow$  \NombreFuncion{\&}($e$)
  	\STATE for $i$=0 to |$c$| do:
