...
\begin{Representacion}
  ...
  \Rep[estr\_acat][e]{ \\
    ...
    ($\forall c$: categoria)(def?($c$, $e$.categorías) $\impluego$ ( \hfill 3. \\
      ...
      \margen ($\forall h$: puntero(estr\_cat))($h$ $\in$ $cat$.hijos $\impluego$ ( \hfill 3. d) \\
        ...
        \margen\margen def?($h$\DRef nombre, $e$.categorías) $\yluego$ $h$ $\igobs$ \&(obtener($h$\DRef nombre, $e$.categorías)) \hfill 3. d) 2) \\
      \margen )) \\
  ...
  \tadOperacion{SecuDeTuplas}{secu(puntero(estr\_cat))}{secu(tupla(categoria, nat))}{}
...
\begin{Interfaz}
  ...
  [Siguientes($res$) podrá cambiar si se agregan links a la categoría $c$ o alguna subcategoría y/o si se registran accesos a links de dichas categorías.]  
...
\begin{Representacion}
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \end{enumerate}
      \item La fecha del último acceso debe ser menor o igual a la fecha actual del sistema.
      \item $ordenado?$ implica que la lista de links está ordenada según
            linksOrdenadosPorAccesos($s$,~$cat$), donde $s$ es el sistema actual y
            $cat$ es el campo $cat$ del elemento actual.
    \end{enumerate}
  ...
  \Rep[estr\_sistema][s]{\\
    ...
    ($\forall i$: nat)($i$ $<$ tam($s$.linksPorCatId) $\impluego$ ( \hfill 3. \\
      ...
      \margen )) $\land$ \\
      \margen $estr\_c$.últimoAcceso $\leq$ $s$.fechaActual $\land$ \hfill 3. f) \\
      \margen $estr\_c$.ordenado? $\impluego$ LinksOrdenados?($estr\_c$) \hfill 3. g) \\
    )) $\land$ \\
  ...
  \tadOperacion{LinksOrdenados?}{estr\_linksPorCatId}{bool}{}
  \tadAxioma{LinksOrdenados?($estr\_c$)}{\textbf{true} $\ssi$ los links están ordenados según linksOrdenadosPorAccesos($s$, $estr\_c$.cat),
                                         donde $s$ es el sistema al cual pertenece $estr\_c$.}
  
  ~  
  
  DUDA: ¿Podemos expresar este auxiliar en castellano?  
  ...
  \tadAxioma{MismosAccesos($l$, $s$, $e$)}{
    accesosRecientesDia($s$, $l$, fechaUltimoAcceso($s$, $l$)) $\igobs$ obtener($l$, $e$.links).as[0] $\land$ \\
    fechaUltimoAcceso($s$, $l$) $\geq$ 1 $\impluego$ \\
      \margen accesosRecientesDia($s$, $l$, fechaUltimoAcceso($s$, $l$) $-$ $1$) $\igobs$ obtener($l$, $e$.links).as[1] $\land$ \\
    fechaUltimoAcceso($s$, $l$) $\geq$ 2 $\impluego$ \\
      \margen accesosRecientesDia($s$, $l$, fechaUltimoAcceso($s$, $l$) $-$ $2$) $\igobs$ obtener($l$, $e$.links).as[2]
  }
