...
\begin{Interfaz}
  ...
  [está?($padre$, $ac$)]
  {$res$ $\igobs$ $\langle$iter: CrearItUni(conjASecu(hijos($ac$, $padre$))), acat: $ac\rangle$ $\land$ aliasingHijos($res$, $padre$)}
  [$\Theta(|padre|)$]
  [devuelve un iterador unidireccional de las categorías hijas directas de la categoría $padre$.]
  [Siguientes($res$.iter) podrá cambiar si se agregan nuevas categorías hijas directas a la categoría $padre$.]

  \InterfazFuncion{CrearIterHijosDeLaRaíz}{\In{ac}{acat}}{itercathijos}
  {$res$ $\igobs$ $\langle$iter: CrearItUni(conjASecu(hijos($ac$, raíz($ac$))), acat: $ac\rangle$ $\land$ aliasingHijos($res$, raíz($ac$))}
  [$\Theta(1)$]
  [devuelve un iterador unidireccional de las categorías hijas directas de la categoría raíz de $ac$.]
  [Siguientes($res$.iter) podrá cambiar si se agregan nuevas categorías hijas directas a la categoría raíz.]

  \InterfazFuncion{CrearIterHijosDadoOtroIter}{\In{it}{itercathijos}}{itercathijos}
  [HayMás?($it$.iter)]
  {$res$ $\igobs$ $\langle$iter: CrearItUni(conjASecu(hijos($it$.acat, Actual($it$.iter)))), acat: $it$.acat$\rangle$ $\land$ \\
  \hspace*{13mm} aliasingHijos($res$, Actual($it$.iter))}  
  [$\Theta(1)$]
  [devuelve un iterador unidireccional de las categorías hijas directas de la categoría actual del iterador $it$.]
  [Siguientes($res$.iter) podrá cambiar si se agregan nuevas categorías hijas directas a la categoría actual del iterador $it$.]
  ...
  [HayMás?($it$.iter)]
  {$res$ $\igobs$ id(Actual($it$.iter), $it$.acat)}
  [$\Theta(1)$]
  [devuelve el id del elemento actual del iterador.]

  \InterfazFuncion{AvanzarHijo}{\Inout{it}{itercathijos}}{}
  [$it$ $\igobs$ $it_{0}$ $\land$ HayMás?($it_{0}$.iter)]
  {$it$ $\igobs$ $\langle$iter: Avanzar($it_{0}$.iter), acat: $it_{0}.acat\rangle$}
  [$\Theta(1)$]
  ...
  {$res$ $\igobs$ $\langle$iter: CrearItUni(ancestros($it$.acat, Actual($it$.iter))), acat: $it$.acat$\rangle$}
  [$\Theta(1)$]
  ...
  {$res$ $\igobs$ $\langle$iter: CrearItUni(ancestros($ac$, raíz($ac$))), acat: $ac\rangle$}
  [$\Theta(1)$]
  ...
  
  {
    ...
    \begin{tad}{\tadNombre{Árbol Extendido}}
      ...
      \tadAlinearFunciones{aliasingHijos}{tupla(itUni(categoría){, }acat),categoría}
      \tadOperacion{conjASecu}{conj(categoría)}{secu(categoría)}{}
      \tadOperacion{secuAConj}{secu(categoría)}{conj(categoría)}{}  
      \tadOperacion{aliasingHijos}{tupla(itUni(categoría){, }acat),categoría}{bool}{}
      \tadOperacion{ancestros}{acat,categoría}{secu(categoría)}{}

      \tadAxiomas[\paratodo{categoría}{c}, \paratodo{acat}{ac}, \paratodo{conj(categoría)}{cc},
                  \paratodo{secu(categoría)}{sc}, \paratodo{tupla(itUni(categoría){, }acat)}{t}]  
  
      \tadAlinearAxiomas{aliasingHijos($t$, $c$)}
      \tadAxioma{conjASecu($cc$)}{\TADIF $\emptyset$?($cc$) THEN \secuencia{} ELSE \secuencia{dameUno($cc$)}[conjASecu(sinUno($cc$))] FI}
      \tadAxioma{secuAConj($sc$)}{\TADIF vacía?($sc$) THEN $\emptyset$ ELSE Ag(prim($sc$), secuAConj(fin($sc$))) FI} 
      \tadAxioma{aliasingHijos($t$, $c$)}{alias(secuAConj(Siguientes($\Pi_1$($t$))) $\subseteq$ hijos($\Pi_2$($t$), $c$))}
      \tadAxioma{ancestros($ac$, $c$)}{\TADIF raíz($ac$) $\igobs$ $c$ THEN \secuencia{$c$} ELSE \secuencia{$c$}[ancestros($ac$, padre($ac$, $c$))] FI}  
    \end{tad}
  }
...
\begin{Representacion}
  ...
  \begin{Estructura}{itercathijos}[itConj(puntero(estr\_cat))]
  \end{Estructura}
...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iCrearIterHijos}{\In{padre}{categoria}, \In{ac}{acat}}{itConj(puntero(estr\_cat))}
    \STATE $res$ $\leftarrow$ \NombreFuncion{CrearIt}(\NombreFuncion{Obtener}($padre$, $ac$.categorías).hijos)
  \end{Algoritmo}

  ~

  \begin{Algoritmo}{iCrearIterHijosDeLaRaíz}{\In{ac}{acat}}{itConj(puntero(estr\_cat))}
    \STATE $res$ $\leftarrow$ \NombreFuncion{CrearIt}($ac$.raíz.hijos)
  \end{Algoritmo}

  ~
  
  \begin{Algoritmo}{iCrearIterHijosDadoOtroIter}{\In{it}{itConj(puntero(estr\_cat))}}{itConj(puntero(estr\_cat))}
    \STATE $res$ $\leftarrow$ \NombreFuncion{CrearIt}(\NombreFuncion{Siguiente}(it)\DRef hijos)
  \end{Algoritmo}  

  ~  
  
  \begin{Algoritmo}{iHayMásHijos?}{\In{it}{itConj(puntero(estr\_cat))}}{bool}
    \STATE $res$ $\leftarrow$ \NombreFuncion{HaySiguiente}($it$)
  \end{Algoritmo}
  
  ~
  
  \begin{Algoritmo}{iHijoActual}{\In{it}{itConj(puntero(estr\_cat))}}{categoria}
    \STATE $res$ $\leftarrow$ \NombreFuncion{Siguiente}($it$)\DRef nombre
  \end{Algoritmo}

  ~
  
  \begin{Algoritmo}{iIdHijoActual}{\In{it}{itConj(puntero(estr\_cat))}}{nat}
    \STATE $res$ $\leftarrow$ \NombreFuncion{Siguiente}($it$)\DRef id
  \end{Algoritmo}  
  
  ~
  
  \begin{Algoritmo}{iAvanzarHijo}{\Inout{it}{itConj(puntero(estr\_cat))}}{}
    \STATE \NombreFuncion{Avanzar}($it$)
  \end{Algoritmo}
  
  ~
  
  \begin{Algoritmo}{iCrearIterAncestros}{\In{it}{itConj(puntero(estr\_cat))}}{puntero(estr\_cat)}
    \STATE $res$ $\leftarrow$ \NombreFuncion{Siguiente}(it)
  \end{Algoritmo}
...
\begin{Interfaz}
  ...
  [está?($c$, categorías($s$))]
  {$res$ $\igobs$ $\langle$iter: CrearItUni(linksOrdenadosPorAccesos($s$, $c$)), sis: $s\rangle$ $\land$ aliasingLinks($res$, $c$)}  
  ...
  
  {
    ...
    \begin{tad}{\tadNombre{Sistema Extendido}}
      ...
      \tadAlinearFunciones{terminanIgual}{tupla(itUni(link){, }lli),categoría}
      \tadOperacion{aliasingLinks}{tupla(itUni(link){, }lli),categoría}{bool}{}
      ...
      \tadAlinearAxiomas{terminanIgual($ls$, $ls'$)}
	  \tadAxioma{aliasingLinks($t$, $c$)}{alias(terminanIgual(Siguientes($\Pi_1$($t$)), linksOrdenadosPorAccesos($\Pi_2$($t$), $c$)))}
...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iCrearSistema}{\In{ac}{estr\_acat}}{estr\_sistema}
    ...
    \STATE \begin{CrearTupla}{$res$.linksPorCatId[0]}
             \CampoTupla{itcat}{\NombreFuncion{CrearIterAncestrosDeLaRaíz}($as$)},
             \CampoTupla{links}{\NombreFuncion{Vacía}()},\\
             \CampoTupla{últimoAcceso}{0},
             \CampoTupla{ordenado?}{\textbf{false}}%
           \end{CrearTupla}
    \STATE \NombreFuncion{iAgregarALinksPorCatId(CrearIterHijosDeLaRaíz($ac$), $res$)}   
  \end{Algoritmo}
  ...
  \begin{Algoritmo}{iAgregarALinksPorCatId}{\Inout{it}{itercathijos}, \Inout{s}{estr\_sistema}}{}
    \WHILE{\NombreFuncion{HayMásHijos?}($it$)}
      \STATE \begin{CrearTupla}{$s$.linksPorCatId[\NombreFuncion{IdHijoActual}($it$) - 1]}
               \CampoTupla{itcat}{\NombreFuncion{CrearIterAncestros}($it$)},
               \CampoTupla{links}{\NombreFuncion{Vacía}()},\\
               \CampoTupla{últimoAcceso}{0},
               \CampoTupla{ordenado?}{\textbf{false}}%
             \end{CrearTupla}
      \STATE \NombreFuncion{iAgregarALinksPorCatId}(\NombreFuncion{CrearIterHijosDadoOtroIter}($it$), $s$)
      \STATE \NombreFuncion{AvanzarHijo}($it$)
    \ENDWHILE
  \end{Algoritmo}
  ...
  \begin{Algoritmo}{iAgregarLink}{\In{l}{link}, \In{c}{categoria}, \Inout{s}{estr\_sistema}}{}
 	...
 	\STATE
    \STATE var $it$: \TipoVariable{itcatancestros}
    \STATE $it$ $\leftarrow$ \NombreFuncion{CopiarIterAncestros}($s$.linksPorCatId[$estr\_l$.cid - 1].itcat)
 	\STATE
 	\WHILE{\NombreFuncion{HayMásAncestros?}($it$)}
      \STATE \NombreFuncion{AgregarAtrás}($s$.linksPorCatId[\NombreFuncion{IdAncestroActual}($it$) - 1].links, \&($estr\_l$))
 	  \STATE $s$.linksPorCatId[\NombreFuncion{IdAncestroActual}($it$) - 1].últimoAcceso $\leftarrow$ $s$.fechaActual
 	  \STATE $s$.linksPorCatId[\NombreFuncion{IdAncestroActual}($it$) - 1].ordenado? $\leftarrow$ \textbf{false}
 	  \STATE \NombreFuncion{AvanzarAncestro}($it$)
 	\ENDWHILE
  \end{Algoritmo}
  ...
  \begin{Algoritmo}{iCrearIterLinks2}{\In{c}{categoría}, \In{s}{estr\_sistema}}{estr\_iter}
    \STATE var $estr\_c$: \TipoVariable{estr\_linksPorCatId}
  	...
  	\STATE
  	\STATE $maxPointer$: \TipoVariable{puntero($estr\_link$)}
  	\STATE $itRecorreredor$: \TipoVariable{itLista(puntero($estr\_link$))} $\leftarrow$ \NombreFuncion{CrearIt}($estr\_c$.links)
  	\STATE $itLista$:  \TipoVariable{itLista(puntero($estr\_link$))} $\leftarrow$ \NombreFuncion{CrearIt}($estr\_c$.links)
  	\IF{$¬$($estr\_c$.ordenado?)}
  	  \WHILE{\NombreFuncion{HaySiguiente}($itLista$)}
  	  	\STATE $itRecorreredor$ $\leftarrow$ \NombreFuncion{Copia}($itLista$)
  	  	\STATE \NombreFuncion{Avanzar($itRecorreredor$)}
  	  \WHILE{\NombreFuncion{HaySiguiente}($itRecorreredor$)} \IF{(AccesosRecientes2(*(\NombreFuncion{Siguiente}($itRecorredor$)), $estr\_c$.últimoAcceso) > \\ (AccesosRecientes2(*($maxPointer$), $estr\_c$.últimoAcceso)}
  	  	\STATE $maxPointer$ $\leftarrow$ \NombreFuncion{Siguiente}($itRecorredor$)
  	  	\ENDIF
		\STATE \NombreFuncion{Avanzar($itRecorredor$)}
	  \ENDWHILE  	  	
  	  \STATE \NombreFuncion{Siguiente}($itLista$) $\leftarrow$ $maxPointer$
  	  \STATE \NombreFuncion{Avanzar($itLista$)}
  	\ENDWHILE
  	\STATE $estr\_c$.ordenado? $\leftarrow$ \textbf{true}
  	\ENDIF
  	\STATE
    \STATE $res$ $\leftarrow$ $\langle$\TipoVariable{s}: $s$, \TipoVariable{c}: $c$, $it$: \NombreFuncion{CrearIt}($estr\_c$.links)$\rangle$
  \end{Algoritmo}  
  
  
  ~  

  \begin{Algoritmo}{iAccesosRecientes2}{\In{estr\_l}{estr\_link}, \In{ultimoAccesoASistema}{nat}}{nat}
    \IF{$ultimoAccesoASistema$ $\igobs$ $estr\_l$.últimoAcceso}
      \STATE $res$ $\leftarrow$ $estr\_l$.as[0] + $estr\_l$.as[1] + $estr\_l$.as[2]
    \ELSIF{$ultimoAccesoASistema$ $\igobs$ $estr\_l$.últimoAcceso + 1}
      \STATE $res$ $\leftarrow$ $estr\_l$.as[0] + $estr\_l$.as[1]
    \ELSIF{$ultimoAccesoASistema$ $\igobs$ $estr\_l$.últimoAcceso + 2}          
  ...
  \begin{Algoritmo}{iOrdenarLinks}{\Inout{estr\_c}{estr\_linksPorCatId}}{}
    \STATE var $arr$: \TipoVariable{arreglo\_dimensionable de puntero(estr\_link)}
    \STATE $arr$ $\leftarrow$ \NombreFuncion{CrearArreglo}(\NombreFuncion{iConvertirListaDeLinksAArreglo}($estr\_c$.links))
    \STATE var $i$: \TipoVariable{nat}    
    \STATE $i$ $\leftarrow$ 0
    \WHILE{$i$ $<$ \NombreFuncion{Longitud}($estr\_c$.links)}
      \STATE var $max$: \TipoVariable{nat}
      \STATE $max$ $\leftarrow$ \NombreFuncion{iIndiceLinkConMásAccesos}($arr$, $i$, $estr\_c$)
      \STATE var $maxPtr$: \TipoVariable{puntero(estr\_link)}
      \STATE $maxPtr$ $\leftarrow$ $arr$[$max$]
      \STATE $arr$[$max$] $\leftarrow$ $arr$[$i$]
      \STATE $arr$[$i$] $\leftarrow$ $maxPtr$
      \STATE $i$ $\leftarrow$ $i$ + 1
    \ENDWHILE
  	\STATE $estr\_c$.links $\leftarrow$ \NombreFuncion{iConvertirArregloAListaDeLinks}($arr$, \NombreFuncion{Longitud}($estr\_c$.links))
  	\STATE $estr\_c$.ordenado? $\leftarrow$ \textbf{true}
  \end{Algoritmo}  
  
  ~
  
  \begin{Algoritmo}{iConvertirListaDeLinksAArreglo}{\In{links}{lista(puntero(estr\_link))}}{arreglo\_dimensionable de puntero(estr\_link)}    
    \STATE var $arr$: \TipoVariable{arreglo\_dimensionable de puntero(estr\_link)}
    \STATE $arr$ $\leftarrow$ \NombreFuncion{CrearArreglo}(\NombreFuncion{Longitud}($links$))
    \STATE var $it$: \TipoVariable{itLista(puntero(estr\_link))}
    \STATE $it$ $\leftarrow$ \NombreFuncion{CrearIt}($links$)
    \STATE var $i$: \TipoVariable{nat}
    \STATE $i$ $\leftarrow$ 0
    \WHILE{\NombreFuncion{HaySiguiente}($it$)}
      \STATE $arr$[$i$] $\leftarrow$ \NombreFuncion{Siguiente}($it$)
      \STATE $i$ $\leftarrow$ $i$ + 1
      \STATE \NombreFuncion{Avanzar}($it$)
    \ENDWHILE
    \STATE $res$ $\leftarrow$ $arr$
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iIndiceLinkConMásAccesos}{\In{arr}{arreglo\_dimensionable de puntero(estr\_link))}, \In{desde}{nat}, \In{estr\_c}{estr\_linksPorCatId}}{nat}
    \STATE var $max$: \TipoVariable{nat}
    \STATE $max$ $\leftarrow$ $desde$
    \STATE var $i$: \TipoVariable{nat}
    \STATE $i$ $\leftarrow$ $desde$
    \WHILE{$i$ $<$ \NombreFuncion{Longitud}($estr\_c$.links)}
      \IF{\NombreFuncion{iAccesosRecientes}(*($arr$[$i$]), $estr\_c$) $>$ \NombreFuncion{iAccesosRecientes}(*($arr$[$max$]), $estr\_c$)}
        \STATE $max$ $\leftarrow$ $i$
      \ENDIF
      \STATE $i$ $\leftarrow$ $i$ + 1
    \ENDWHILE
    \STATE $res$ $\leftarrow$ $max$
  \end{Algoritmo}
  ...
  \begin{Algoritmo}{iAccesosRecientes}{\In{estr\_l}{estr\_link}, \In{estr\_c}{estr\_linksPorCatId}}{nat}
    ...
    \ELSIF{$estr\_c$.últimoAcceso $\igobs$ $estr\_l$.últimoAcceso + 1}
      \STATE $res$ $\leftarrow$ $estr\_l$.as[1] + $estr\_l$.as[2]
    \ELSIF{$estr\_c$.últimoAcceso $\igobs$ $estr\_l$.últimoAcceso + 2}          
      \STATE $res$ $\leftarrow$ $estr\_l$.as[2]
    \ELSE
      \STATE $res$ $\leftarrow$ 0
    \ENDIF
  \end{Algoritmo}
  ...
  \begin{Algoritmo}{iConvertirArregloAListaDeLinks}{\In{arr}{arreglo\_dimensionable de puntero(estr\_link)}, \In{n}{nat}}{lista(puntero(estr\_link))}    
  	\STATE var $links$: \TipoVariable{lista(puntero(estr\_link))}
  	\STATE $links$ $\leftarrow$ \NombreFuncion{Vacía}()
  	\STATE var $i$: \TipoVariable{nat}
  	\STATE $i$ $\leftarrow$ $0$
  	\WHILE{$i$ $<$ $n$}
  	  \STATE \NombreFuncion{AgregarAtras}($links$, $arr$[$i$])
  	  \STATE $i$ $\leftarrow$ $i$ + 1
  	\ENDWHILE
    \STATE $res$ $\leftarrow$ $links$
  \end{Algoritmo}
  ...
  \begin{Algoritmo}{iCategoríaLinkActual}{\In{it}{estr\_iter}}{categoria}
    $res$ $\leftarrow$ \NombreFuncion{AncestroActual}($it$.s.linksPorCatId[\NombreFuncion{Siguiente}($it$.it)\DRef cid - 1].itcat))
  \end{Algoritmo}
  
  ~  
  
  \begin{Algoritmo}{iAccesosRecientesLinkActual}{\In{it}{estr\_iter}}{nat}
    \STATE $res$ $\leftarrow$ \NombreFuncion{iAccesosRecientes}(*Actual($it$.iter), $it$.s.linksPorCatId[$it$.cid - 1])
  \end{Algoritmo}
