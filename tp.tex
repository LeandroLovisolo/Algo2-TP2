...
\begin{Interfaz}
  ...
  {$it$ $\igobs$ $\langle$iter: Avanzar($it_{0}$.iter), acat: $it_{0}.acat\rangle$ $\land$ aliasingHijos($it$, padre($it_{0}$.acat, Actual($it_{0}$.iter)))}
  [$\Theta(1)$]
  [avanza el iterador a la posición siguiente.]
  [Siguientes($it$.iter) podrá cambiar si se agregan nuevas categorías hijas directas a la categoría padre de las categorías que itera $it$.]
