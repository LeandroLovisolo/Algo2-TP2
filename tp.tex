...
\begin{Interfaz}
  ...
  \InterfazFuncion{CrearIterAncestros}{\In{c}{categoria}, \In{ac}{acat}}{itercatancestros}
  [está?($c$, $ac$)]
  {esPermutacion?(Siguientes($res$), ancestros($ac$, $c$))}
  [$\Theta(|c|)$]
  [devuelve un iterador unidireccional de la categoría $c$ y sus ancestros.]
...
\begin{Representacion}
  ...
  \begin{Estructura}{acat}[estr\_acat]
    \begin{Tupla}[estr\_acat]
      \tupItem{raíz}{puntero(estr\_datoscat)}%
      \tupItem{categorías}{dicctrie(estr\_datoscat)}%
    \end{Tupla}
    \begin{Tupla}[estr\_datoscat]
      \tupItem{id}{nat}
      \tupItem{nombre}{categoria}
      \tupItem{padre}{puntero(estr\_datoscat)}
      \tupItem{hijos}{conj(puntero(estr\_datoscat))}
    \end{Tupla}    
  \end{Estructura}
  ...
  \begin{Estructura}{itercathijos}[itConj(datoscat)]
  \end{Estructura}
  ...
  \begin{Estructura}{itercatancestros}[puntero(estr\_datoscat)]
  \end{Estructura} 
...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iCrearSistema}{\In{ac}{estr\_acat}}{estr\_sistema}
    ...
    \STATE
    \STATE PENDIENTE: Inicializar arreglo linksPorCatId.
