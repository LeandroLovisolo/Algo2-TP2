...
\begin{Algoritmos}
  ...
  \begin{Algoritmo}{iAgregarLink}{\In{l}{link}, \In{c}{categoria}, \Inout{s}{estr\_sistema}}{}
    ...
    \STATE $it$ $\leftarrow$ \NombreFuncion{CopiarIterAncestros}($s$.linksPorCatId[$estr\_l$.cid].itcat)
 	\STATE
 	\WHILE{\NombreFuncion{HayMásAncestros?}($it$)}
      \STATE \NombreFuncion{AgregarAtrás}($s$.linksPorCatId[\NombreFuncion{IdAncestroActual}($it$)].links, $estr\_l$)
 	  \STATE $s$.linksPorCatId[\NombreFuncion{IdAncestroActual}($it$)].últimoAcceso $\leftarrow$ $s$.fechaActual
 	  \STATE $s$.linksPorCatId[\NombreFuncion{IdAncestroActual}($it$)].ordenado? $\leftarrow$ \textbf{false}
 	\ENDWHILE  	
  \end{Algoritmo}
