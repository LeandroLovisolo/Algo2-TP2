...
\tadAxioma{categoríasADatosCat($cs$, $ac$)}{\IF $\emptyset$?($cs$)
                                            THEN $\emptyset$
...
\begin{Interfaz}
  ...
  DUDA: ¿Hace falta crear un TAD para el género datoscat para poder especificar
        las pre y postcondiciones de las funciones a continuación?
...
\begin{Interfaz}
  ...
  \Titulo{Operaciones básicas de diccionario trie}
   ...
   \InterfazFuncion{Crear}{}{dicctrie$(\alpha)$}
  [true]
  {$res$ $\igobs$ vacío()}
  [$\Theta(1)$]
  [crea un diccionario de trie]

   \InterfazFuncion{Definido}{\In{t}{dicctrie$(\alpha)$},  \In{clave}{string}}{bool}
  [true]
  {$res$ $\igobs$ def?($clave$, $t$)}
  [$\Theta(|clave|)$]
  [devuelve true si está definido $clave$.]

   \InterfazFuncion{Definir}{\Inout{t}{dicctrie$(\alpha)$},  \In{clave}{string}, \In{sig}{$\alpha$}}{}
  [$t$ $\igobs$ $t_{0}$]
  {$t$ $\igobs$ definir($t_{0}$, $clave$, $sig$)}
  [$\Theta(|clave| + sig)$]
  [define la clave $clave$ con el significado $sig$.]
  
  DUDA Como poner la complejidad de sig? 
  
   \InterfazFuncion{Obtener}{\In{t}{dicctrie$(\alpha)$}, \In{clave}{string}}{$\alpha$}
  [def?($clave$, $t$)]
  {$res$ $\igobs$ obtener($clave$, $t$)}
  [$\Theta(|clave|)$]
  [obtiene el significado definido como $clave$.]  
...
\begin{Representacion}
  ...
  \begin{Estructura}{dicctrie$(\alpha)$}[estr\_dicctrie]
    \begin{Tupla}[estr\_dicctrie	]
      \tupItem{hayClave}{bool}
      \tupItem{nodos}{arreglo\_estático[255] de puntero(estr\_dicctrie)}
